...
if __name__ == "__main__":
    ...
    interaction_file_list = [
                                "interaction_1_1000_False",
                                "interaction_2_1000_False",
                                "interaction_3_1000_False",
                            ]
    for tmp_file_name in interaction_file_list:
        f = open("../cache/%s" %tmp_file_name, "r")
        for lines in f:
            for ind_way_2 in range(2):
                feat = lines.strip().split(",")
                if ind_way_2 == 1:
                    feat.reverse()
                feat_name = '_'.join(feat)
                print feat_name
                if len(feat) == 2:
                    train_test[feat_name] = (
                                             train_test[feat[0]].apply(str) +
                                             train_test[feat[1]].apply(str)
                                            )
                elif len(feat) == 3:
                    train_test[feat_name] = (
                                             train_test[feat[0]].apply(str) +
                                             train_test[feat[1]].apply(str) +
                                             train_test[feat[2]].apply(str)
                                            )
                elif len(feat) == 4:
                    train_test[feat_name] = (
                                             train_test[feat[0]].apply(str) +
                                             train_test[feat[1]].apply(str) +
                                             train_test[feat[2]].apply(str) +
                                             train_test[feat[3]].apply(str)
                                            )
                train_test[feat_name] = train_test[feat_name].apply(encode)
        f.close()
