...
print "XGB_retrain_2__%s.csv"%sys.argv[1]
print "$$$$"
...
if __name__ == "__main__":
    ...
    np.random.seed(6174)
    k=0
    for comb in itertools.combinations(COMB_FEATURE, 4):
        if np.random.uniform(0,1) < 0.993:
            print "#"
            continue
        k+=1
    ...
    submission.to_csv("XGB_retrain_2__%s.csv"%sys.argv[1], index_label='id')
    ########################################################################################
