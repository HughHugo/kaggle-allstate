...
pred_nn_retrain_sd = gmean([pred_nn_1_retrain.values,
                pred_nn_2_retrain.values,
                pred_nn_3_retrain.values,
                pred_nn_4_retrain.values,
                pred_nn_5_retrain.values,
...
pred_nn_sd = gmean([pred_nn_1['loss'],
                     pred_nn_2['loss'],
                     pred_nn_3['loss'],
                     pred_nn_4['loss'],
                     pred_nn_5['loss'],
...
pred_xgb_retrain_sd = gmean([pred_xgb_1_retrain.values,
                pred_xgb_2_retrain.values,
                pred_xgb_3_retrain.values,
                pred_xgb_6_retrain.values,
                pred_xgb_9_retrain.values,
...
pred_xgb_sd = gmean([pred_xgb_1['loss'],
                      pred_xgb_2['loss'],
                      pred_xgb_3['loss'],
                      pred_xgb_6['loss'],
                      pred_xgb_9['loss'],
...
args = [
    ...
    pred_nn_retrain,                            #33
    pred_xgb_retrain,                           #34
    pred_nn_3_retrain['loss'].values - pred_xgb_10_retrain['loss'].values,
    pred_xgb_17_retrain['loss'].values - pred_new_nn_4_retrain['loss'].values,
    pred_xgb_15_retrain['loss'].values - pred_nn_4_fix_retrain['loss'].values,
    train['cont4'],
    train['cont5'],
    train['cont6'],
    train['cont7'],
...
def f(coord,args):
    ...
    return np.mean( np.abs(coord[pe*0]*args[0]# + coord[pe*0+2]*np.log(200 + args[0])# + coord[pe*0+3]*1/(1.0+args[0]) + coord[pe*0+4]*(args[0] ** 0.5)
     + coord[pe*1]*args[1] #+ coord[pe*1+2]*np.log(200 + args[1]) #+ coord[pe*1+3]*1/(1.0+args[1]) + coord[pe*1+4]*(args[1] ** 0.5)
     + coord[pe*2]*args[2] #+ coord[pe*2+2]*np.log(200 + args[2]) #+ coord[pe*2+3]*1/(1.0+args[2]) + coord[pe*2+4]*(args[2] ** 0.5)
     + coord[pe*3]*args[3] #+ coord[pe*3+2]*np.log(200 + args[3]) #+ coord[pe*3+3]*1/(1.0+args[3]) + coord[pe*3+4]*(args[3] ** 0.5)
     + coord[pe*4]*args[4] #+ coord[pe*4+2]*np.log(200 + args[4]) #+ coord[pe*4+3]*1/(1.0+args[4]) + coord[pe*4+4]*(args[4] ** 0.5)
     + coord[pe*5]*args[5] #+ coord[pe*5+2]*np.log(200 + args[5]) #+ coord[pe*5+3]*1/(1.0+args[5]) + coord[pe*5+4]*(args[5] ** 0.5)
     + coord[pe*6]*args[6] #+ coord[pe*6+2]*np.log(200 + args[6]) #+ coord[pe*6+3]*1/(1.0+args[6]) + coord[pe*6+4]*(args[6] ** 0.5)
     + coord[pe*7]*args[7] #+ coord[pe*7+2]*np.log(200 + args[7]) #+ coord[pe*7+3]*1/(1.0+args[7]) + coord[pe*7+4]*(args[7] ** 0.5)
     + coord[pe*8]*args[8] #+ coord[pe*8+2]*np.log(200 + args[8]) #+ coord[pe*8+3]*1/(1.0+args[8]) + coord[pe*8+4]*(args[8] ** 0.5)
     + coord[pe*9]*args[9] #+ coord[pe*9+2]*np.log(200 + args[9]) #+ coord[pe*9+3]*1/(1.0+args[9]) + coord[pe*9+4]*(args[9] ** 0.5)
     + coord[pe*10]*args[10] #+ coord[pe*10+2]*np.log(200 + args[10]) #+ coord[pe*10+3]*1/(1.0+args[10]) + coord[pe*10+4]*(args[10] ** 0.5)
     + coord[pe*11]*args[11] #+ coord[pe*11+2]*np.log(200 + args[11]) #+ coord[pe*11+3]*1/(1.0+args[11]) + coord[pe*11+4]*(args[11] ** 0.5)
     + coord[pe*12]*args[12] #+ coord[pe*12+2]*np.log(200 + args[12]) #+ coord[pe*12+3]*1/(1.0+args[12]) + coord[pe*12+4]*(args[12] ** 0.5)
     + coord[pe*13]*args[13] #+ coord[pe*13+2]*np.log(200 + args[13]) #+ coord[pe*13+3]*1/(1.0+args[13]) + coord[pe*13+4]*(args[13] ** 0.5)
     + coord[pe*14]*args[14] #+ coord[pe*14+2]*np.log(200 + args[14]) #+ coord[pe*14+3]*1/(1.0+args[14]) + coord[pe*14+4]*(args[14] ** 0.5)
     + coord[pe*15]*args[15] #+ coord[pe*15+2]*np.log(200 + args[15]) #+ coord[pe*15+3]*1/(1.0+args[15]) + coord[pe*15+4]*(args[15] ** 0.5)
     + coord[pe*16]*args[16] #+ coord[pe*16+2]*np.log(200 + args[16]) #+ coord[pe*16+3]*1/(1.0+args[16]) + coord[pe*16+4]*(args[16] ** 0.5)
     + coord[pe*17]*args[17] #+ coord[pe*17+2]*np.log(200 + args[17]) #+ coord[pe*17+3]*1/(1.0+args[17]) + coord[pe*17+4]*(args[17] ** 0.5)
     + coord[pe*18]*args[18] #+ coord[pe*18+2]*np.log(200 + args[18]) #+ coord[pe*18+3]*1/(1.0+args[18]) + coord[pe*18+4]*(args[18] ** 0.5)
     + coord[pe*19]*args[19] #+ coord[pe*19+2]*np.log(200 + args[19]) #+ coord[pe*19+3]*1/(1.0+args[19]) + coord[pe*19+4]*(args[19] ** 0.5)
     + coord[pe*20]*args[20] #+ coord[pe*20+2]*np.log(200 + args[20]) #+ coord[pe*20+3]*1/(1.0+args[20]) + coord[pe*20+4]*(args[20] ** 0.5)
     + coord[pe*21]*args[21] #+ coord[pe*21+2]*np.log(200 + args[21]) #+ coord[pe*21+3]*1/(1.0+args[21]) + coord[pe*21+4]*(args[21] ** 0.5)
     + coord[pe*22]*args[22] #+ coord[pe*22+2]*np.log(200 + args[22]) #+ coord[pe*22+3]*1/(1.0+args[22]) + coord[pe*22+4]*(args[22] ** 0.5)
     + coord[pe*23]*args[23] #+ coord[pe*23+2]*np.log(200 + args[23]) #+ coord[pe*23+3]*1/(1.0+args[23]) + coord[pe*23+4]*(args[23] ** 0.5)
     + coord[pe*24]*args[24] #+ coord[pe*24+2]*np.log(200 + args[24]) #+ coord[pe*24+3]*1/(1.0+args[24]) + coord[pe*24+4]*(args[24] ** 0.5)
     + coord[pe*25]*args[25] #+ coord[pe*25+2]*np.log(200 + args[25]) #+ coord[pe*25+3]*1/(1.0+args[25]) + coord[pe*25+4]*(args[25] ** 0.5)
     + coord[pe*26]*args[26] #+ coord[pe*26+2]*np.log(200 + args[26]) #+ coord[pe*26+3]*1/(1.0+args[26]) + coord[pe*26+4]*(args[26] ** 0.5)
     + coord[pe*27]*args[27] #+ coord[pe*27+2]*np.log(200 + args[27]) #+ coord[pe*27+3]*1/(1.0+args[27]) + coord[pe*27+4]*(args[27] ** 0.5)
     + coord[pe*28]*args[28] #+ coord[pe*28+2]*np.log(200 + args[28]) #+ coord[pe*28+3]*1/(1.0+args[28]) + coord[pe*28+4]*(args[28] ** 0.5)
     + coord[pe*29]*args[29] #+ coord[pe*29+2]*np.log(200 + args[29]) #+ coord[pe*29+3]*1/(1.0+args[29]) + coord[pe*29+4]*(args[29] ** 0.5)
     + coord[pe*30]*args[30] #+ coord[pe*30+2]*np.log(200 + args[30]) #+ coord[pe*30+3]*1/(1.0+args[30]) + coord[pe*30+4]*(args[30] ** 0.5)
     + coord[pe*31]*args[31] #+ coord[pe*31+2]*np.log(200 + args[31]) #+ coord[pe*31+3]*1/(1.0+args[31]) + coord[pe*31+4]*(args[31] ** 0.5)
     ...
     #+ coord[pe*32+7]*args[34] #1
     #+ coord[pe*32+8]*args[35] #2
     #+ coord[pe*32+9]*args[36] #3
     #+ coord[pe*32+10]*args[37] #4
     #+ coord[pe*32+11]*args[38] #5
     #+ coord[pe*32+12]*args[39] #6
     #+ coord[pe*32+13]*args[40] #7
     #+ coord[pe*32+14]*args[41] #8
     #+ coord[pe*32+15]*args[42] #9
     #+ coord[pe*32+16]*args[43] #10
     #+ coord[pe*32+17]*args[44] #11
     #+ coord[pe*32+18]*args[45] #12
     #+ coord[pe*32+19]*args[46] #13
     #+ coord[pe*32+20]*args[47] #14
...
initial_guess = np.array([0.1 for x in range(pe * 32 + 21)])
