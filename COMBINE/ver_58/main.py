...
def f(coord,args):
    ...
    return np.mean( np.abs(coord[pe*0]*args[0] + coord[pe*0+1]*(args[0] ** 2) + coord[pe*0+2]*np.log(args[0]) + coord[pe*0+3]*1/(1.0+args[0]) + coord[pe*0+4]*(args[0] ** 0.5)
     ...
     + coord[pe*36+8]*( args[40] - args[41] )
     + coord[pe*36+9]*( args[42] - args[43] )
     + coord[pe*36+10]*( args[44] - args[45] )
     + coord[pe*36+11]*( args[46] - args[47] )
     + coord[pe*36+12]*( args[48] - args[49] )
...
initial_guess = np.array([0.00001 for x in range(pe * 36 + 13)])
