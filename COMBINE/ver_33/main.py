...
def f(coord,args):
    ...
    return np.mean( np.abs(coord[pe*0]*args[0] + coord[pe*0+1]*(args[0] ** 2) + coord[pe*0+2]*np.log(args[0]) + coord[pe*0+3]*1/(1.0+args[0]) + coord[pe*0+4]*(args[0] ** 0.5)
     ...
     + coord[pe*28+0]*(args[23] - args[16])
     + coord[pe*28+1]*(abs(args[23] - args[16]))
     + coord[pe*28+2]*((args[23]-args[16]) ** 2)
     + coord[pe*28+3]*np.log(abs(args[23]-args[16]))
     + coord[pe*28+4]*1/(1.0+args[23]-args[16])
     + coord[pe*28+5]*1/(1.0+abs(args[23]-args[16]))
     + coord[pe*28+6]*(abs(args[23]-args[16]) ** 0.5)
     + coord[pe*28+7]*(args[26] - args[16])
     + coord[pe*28+8]*(abs(args[26] - args[16]))
     + coord[pe*28+9]*((args[26]-args[16]) ** 2)
     + coord[pe*28+10]*np.log(abs(args[26]-args[16]))
     + coord[pe*28+11]*1/(1.0+args[26]-args[16])
     + coord[pe*28+12]*1/(1.0+abs(args[26]-args[16]))
     + coord[pe*28+13]*(abs(args[26]-args[23]) ** 0.5)
     + coord[pe*28+14]*(args[26] - args[23])
     + coord[pe*28+15]*(abs(args[26] - args[23]))
     + coord[pe*28+16]*((args[26]-args[23]) ** 2)
     + coord[pe*28+17]*np.log(1+abs(args[26]-args[23]))
     + coord[pe*28+18]*1/(1.0+(args[26]-args[23]))
     + coord[pe*28+19]*1/(1.0+abs(args[26]-args[23]))
     + coord[pe*28+20]*(abs(args[26]-args[23]) ** 0.5)
     + coord[pe*28+21]
     - args[-1]) )


initial_guess = np.array([0.1 for x in range(pe * 28 + 7 + 7 + 7 + 1)])
