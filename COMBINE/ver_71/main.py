...
initial_guess = np.array([np.random.uniform(-0.01,0.01) for x in range(pe * 57 + 13)])
...
pred_ensemble = (
     ...
     + res.x[pe*57+0]*( args[-15] - args[-14] )
     + res.x[pe*57+1]*( (args[-15] - args[-14]) ** 2 )
     + res.x[pe*57+2]*( np.log(abs(args[-15] - args[-14])) )
     + res.x[pe*57+3]*( 1/(1.0+args[-15]-args[-14]) )
     + res.x[pe*57+4]*( abs(args[-15]-args[-14]) ** 0.5 )
     + res.x[pe*57+5]*( np.sin(args[-15]-args[-14]) )
     + res.x[pe*57+6]*( np.cos(args[-15]-args[-14]) )
     + res.x[pe*57+7]*( args[-13] - args[-12] )
     + res.x[pe*57+8]*( args[-11] - args[-10] )
     + res.x[pe*57+9]*( args[-9] - args[-8] )
     + res.x[pe*57+10]*( args[-7] - args[-6] )
     + res.x[pe*57+11]*( args[-5] - args[-4] )
     + res.x[pe*57+12]*( args[-3] - args[-2] )
 )
...
args = [
    ...
    pred_new_nn_5_interaction_80['loss'],  #22
    pred_new_nn_5_interaction_90['loss'],  #23
    pred_new_nn_5_20_shift['loss'],        #24
    pred_new_nn_5_100_shift['loss'],       #25
    pred_new_nn_5_2000_shift['loss'],      #26
    pred_new_nn_5_20000_shift['loss'],     #27
    pred_new_nn_6['loss'],                 #28
    pred_new_nn_7['loss'],                 #29
    pred_new_nn_1_65['loss'],              #30
    pred_new_nn_2_65['loss'],              #31
    pred_xgb_1['loss'],                    #32
    pred_xgb_2['loss'],                    #33
    pred_xgb_3['loss'],                    #34
    pred_xgb_6['loss'],                    #35
    ...
    pred_xgb_13['loss'],                   #39
    pred_xgb_14['loss'],                   #40
    pred_xgb_14_obj['loss'],               #41
    pred_xgb_14_obj_2_fast['loss'],        #42
    pred_xgb_15['loss'],                   #43
    pred_xgb_17['loss'],                   #44
    pred_xgb_17_2way['loss'],              #45
    pred_xgb_18['loss'],                   #46
    pred_xgb_19['loss'],                   #47
    pred_xgb_20['loss'],                   #48
    pred_xgb_21['loss'],                   #49
    pred_xgb_22['loss'],                   #50
    pred_xgb_24['loss'],                   #51
    pred_xgb_24_obj_2['loss'],             #52
    pred_xgb_24_obj_fast['loss'],          #53
    pred_xgb_25['loss'],                   #54
    pred_xgb_25_obj_fast['loss'],          #55
    pred_xgb_25_obj_2_fast['loss'],        #56
    pred_r_xgb_1['loss'],                  #57 ####
    pred_nn['loss'],                       #58 14
    pred_xgb['loss'],                      #59 13
    pred_nn_gmean['loss'],                 #60 12
    pred_xgb_gmean['loss'],                #61 11
    pred_nn_sd['loss'],                    #62 10
    pred_xgb_sd['loss'],                   #63 9
    pred_nn_mean_sd_1['loss'],             #64 8
    pred_xgb_mean_sd_1['loss'],            #65 7
    pred_nn_mean_sd_2['loss'],             #66 6
    pred_xgb_mean_sd_2['loss'],            #67 5
    pred_nn_mean_sd_3['loss'],             #68 4
    pred_xgb_mean_sd_3['loss'],            #69 3
    pred_nn_mean_sd_4['loss'],             #70 2
    pred_xgb_mean_sd_4['loss'],            #71 1
    train['loss']
]
