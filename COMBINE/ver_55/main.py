...
args = [
    ...
    pred_xgb_17_2way_retrain['loss'].values,    #32
    pred_xgb_18_retrain['loss'].values,         #33
    pred_xgb_20_retrain['loss'].values,         #34 ####
    pred_nn_retrain,                            #35
    pred_xgb_retrain,                           #36
...
def f(coord,args):
    ...
    return np.mean( np.abs(coord[pe*0]*args[0] + coord[pe*0+1]*(args[0] ** 2) + coord[pe*0+2]*np.log(args[0]) + coord[pe*0+3]*1/(1.0+args[0]) + coord[pe*0+4]*(args[0] ** 0.5)
     ...
     + coord[pe*34+0]*( args[34] - args[35] )
     + coord[pe*34+1]*( (args[34] - args[35]) ** 2 )
     + coord[pe*34+2]*( np.log(abs(args[34] - args[35])) )
     + coord[pe*34+3]*( 1/(1.0+args[34]-args[35]) )
     + coord[pe*34+4]*( abs(args[34]-args[35]) ** 0.5 )
     + coord[pe*34+5]*( np.sin(args[34]-args[35]) )
     + coord[pe*34+6]*( np.cos(args[34]-args[35]) )
