...
def f(coord,args):
    ...
    return np.mean( np.abs(coord[pe*0]*args[0] + coord[pe*0+1]*(args[0] ** 2) + coord[pe*0+2]*np.log(args[0]) + coord[pe*0+3]*1/(1.0+args[0]) + coord[pe*0+4]*(args[0] ** 0.5)
     ...
     + coord[pe*32+7]*( np.tan(args[32]-args[33]) )
     + coord[pe*32+8]*( np.arcsinh(args[32]-args[33]) )
     + coord[pe*32+9]*( np.arctan(args[32]-args[33]) )
     + coord[pe*32+10]*( np.deg2rad(args[32]-args[33]) )
     + coord[pe*32+11]*( np.rad2deg(args[32]-args[33]) )
     + coord[pe*32+12]*( 1/(args[32]+args[33]) )
     + coord[pe*32+13]*( 1/((args[32]-args[33])**2 ))
     + coord[pe*32+14]*( 1/((args[32]*args[33])))
     - args[-1]) )


initial_guess = np.array([0.1 for x in range(pe * 32 +15)])
