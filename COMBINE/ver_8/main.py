...
# ======================== NN optimize ======================== #
def f(coord,args):
    pred_1,pred_2,pred_3,pred_4,pred_5,pred_6,pred_7,pred_8,pred_9,pred_10,pred_11,pred_12,pred_13,pred_14,pred_15,r = args
    return np.mean( np.abs(coord[0]*pred_1 + coord[1]*pred_2 + coord[2]*pred_3
                          ...
                          +coord[9]*pred_10 + coord[10]*pred_11 + coord[11]*pred_12
                          +coord[12]*pred_13 + coord[13]*pred_14 + coord[14]*pred_15 - r))


initial_guess = np.array([0.1 for x in range(15)])
...
pred_ensemble = (res.x[0]*pred_nn_1 + res.x[1]*pred_nn_2 + res.x[2]*pred_nn_3
               + res.x[3]*pred_nn_4 + res.x[4]*pred_nn_5 + res.x[5]*pred_nn_6
               + res.x[6]*pred_nn_1_fix + res.x[7]*pred_nn_2_fix + res.x[8]*pred_nn_3_fix
               + res.x[9]*pred_nn_4_fix + res.x[10]*pred_nn_5_fix + res.x[11]*pred_nn_6_fix
               + res.x[12]*pred_xgb_1 + res.x[13]*pred_xgb_2 + res.x[14]*pred_xgb_3)


pred_ensemble.to_csv("pred_retrain.csv", index_label='id')

tmp = (res.x[0]*pred_nn_1_retrain + res.x[1]*pred_nn_2_retrain + res.x[2]*pred_nn_3_retrain
               + res.x[3]*pred_nn_4_retrain + res.x[4]*pred_nn_5_retrain + res.x[5]*pred_nn_6_retrain
               + res.x[6]*pred_nn_1_fix_retrain + res.x[7]*pred_nn_2_fix_retrain + res.x[8]*pred_nn_3_fix_retrain
               + res.x[9]*pred_nn_4_fix_retrain + res.x[10]*pred_nn_5_fix_retrain + res.x[11]*pred_nn_6_fix_retrain
               + res.x[12]*pred_xgb_1_retrain + res.x[13]*pred_xgb_2_retrain + res.x[14]*pred_xgb_3_retrain)
